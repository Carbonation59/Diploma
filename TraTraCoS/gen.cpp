...
void generate_info(Poco::Data::Session& session){
    ...
    create_stmt2 << "CREATE TABLE CDRs"
                 << "("
                 << "id BIGINT NOT NULL,"
                 << "cur_time BIGINT NOT NULL,"                        // время, когда звонок показался на коммутаторе в миллисекундах
                 << "duration_time INT NOT NULL,"                      // продолжительность звонка в миллисекундах
    ...
    generate_calls(reqs, start_points, end_points);
    /*reqs = {
